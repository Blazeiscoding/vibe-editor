...
export class SuggestionManager {
  ...
  acceptSuggestion(): boolean {
    ...
    try {
      ...
      console.log("Suggestion accepted successfully, new position:", {
        newLine,
        newColumn,
      });
  ...
  isPositionValid(position: any): boolean {
    ...
    const isColumnInRange =
      ...
      position.column <= suggestion.position.column + 10; // More lenient
  ...
  isChangeFromSuggestion(change: any): boolean {
    ...
    return (
      cleanText === this.activeSuggestion.text ||
      this.activeSuggestion.text.startsWith(cleanText)
    );
  }
  ...
  private registerInlineProvider(): void {
    ...
    this.inlineProvider =
      this.monaco.languages.registerInlineCompletionsProvider(language, {
        ...
        // freeInlineCompletions is deprecated/removed in newer Monaco types
        disposeInlineCompletions: (_completions: any) => {},
      });
  }
